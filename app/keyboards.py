...
def main_menu(user=None, has_projects: bool = False, demo_mode: bool = False) -> InlineKeyboardMarkup:
    rows = [
    ...
    if demo_mode:
        rows.insert(1, [InlineKeyboardButton(text="🎭 Voir la démo utilisateur", callback_data="demo:user")])
        rows.insert(2, [InlineKeyboardButton(text="📊 Voir la démo listeur", callback_data="demo:owner")])
    if has_projects:
...
def super_admin_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🎭 Mode démo ON/OFF", callback_data="sa:demo")],
        [InlineKeyboardButton(text="🏠 Menu", callback_data="home")],
    ])
