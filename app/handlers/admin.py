...
async def mod_kb():
    ...
    return kb([
        ...
        [("Warnings catégories", "mod:warning_menu")],
...
async def moderation(message: Message):
    async with SessionLocal() as session:
        if not await admin_only(session, message.from_user):
            return
    await message.answer("Modération", reply_markup=await mod_kb())
...
async def mod_menu(call: CallbackQuery):
    ...
    await call.message.edit_text("Modération", reply_markup=await mod_kb())
    await call.answer()
...
async def toggle_demo(call: CallbackQuery):
    ...
    await call.message.edit_text("Mode démo modifié.", reply_markup=await mod_kb())
    await call.answer()
...
async def pending(call: CallbackQuery):
    ...
    for p in projects:
        ...
        rows.append([(p.title, f"mod:project:{p.id}")])
    rows.append([("Retour", "mod:menu")])
...
async def approve(call: CallbackQuery):
    ...
    async with SessionLocal() as session:
        ...
        if p:
            ...
            if owner:
                try: await call.bot.send_message(owner.telegram_id, f"Ton groupe {p.title} a été approuvé.")
                except Exception: pass
    await call.message.edit_text("Projet approuvé.", reply_markup=kb([[("Retour", "mod:pending")]]))
    await call.answer()
...
async def reject_start(call: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(AdminState.reject_project)
...
async def reject_finish(message: Message, state: FSMContext):
    ...
    async with SessionLocal() as session:
        ...
        if p:
            ...
            await session.commit()
            if owner:
                try: await message.bot.send_message(owner.telegram_id, f"Ton listing {p.title} a été refusé.\n\nMotif :\n{message.text}")
                except Exception: pass
    await state.clear()
...
async def add_cat_start(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text("Nom de la catégorie :", reply_markup=cancel_kb())
    await call.answer()
...
async def delete_cat_menu(call: CallbackQuery):
    ...
    rows = [[(c.name, f"mod:delete_cat:{c.id}")] for c in cats]
    rows.append([("Retour", "mod:menu")])
    await call.message.edit_text("Supprimer une catégorie supprimera aussi les infos liées.", reply_markup=kb(rows))
    await call.answer()
...
async def delete_cat(call: CallbackQuery):
    ...
    async with SessionLocal() as session:
        ...
        if cat:
            ...
            await session.commit()
    await call.message.edit_text("Catégorie supprimée.", reply_markup=kb([[("Modération", "mod:menu")]]))
    await call.answer()

@router.callback_query(F.data == "mod:warning_menu")
async def warning_menu(call: CallbackQuery):
    async with SessionLocal() as session:
        cats = list((await session.scalars(select(Category).order_by(Category.name))).all())
    rows = [[(f"{c.name} [{'ON' if c.warning_enabled else 'OFF'}]", f"mod:warning:{c.id}")] for c in cats]
    rows.append([("Retour", "mod:menu")])
    await call.message.edit_text("Warnings catégories :", reply_markup=kb(rows))
    await call.answer()

@router.callback_query(F.data.startswith("mod:warning:"))
async def warning_config(call: CallbackQuery):
    cid = int(call.data.split(":")[2])
    async with SessionLocal() as session:
        cat = await session.get(Category, cid)
    await call.message.edit_text(
        f"Catégorie : {cat.name}\nActivé : {'Oui' if cat.warning_enabled else 'Non'}\n\nMessage :\n{cat.warning_text or 'Aucun'}",
        reply_markup=kb([
            [("Activer", f"mod:warning_enable:{cid}")],
            [("Désactiver", f"mod:warning_disable:{cid}")],
            [("Modifier texte", f"mod:warning_edit:{cid}")],
            [("Retour", "mod:warning_menu")]
        ])
    )
    await call.answer()

@router.callback_query(F.data.startswith("mod:warning_enable:"))
async def warning_enable(call: CallbackQuery):
    cid = int(call.data.split(":")[2])
    async with SessionLocal() as session:
        cat = await session.get(Category, cid)
        cat.warning_enabled = True
        await session.commit()
    await call.answer("Activé", show_alert=True)

@router.callback_query(F.data.startswith("mod:warning_disable:"))
async def warning_disable(call: CallbackQuery):
    cid = int(call.data.split(":")[2])
    async with SessionLocal() as session:
        cat = await session.get(Category, cid)
        cat.warning_enabled = False
        await session.commit()
    await call.answer("Désactivé", show_alert=True)

@router.callback_query(F.data.startswith("mod:warning_edit:"))
async def warning_edit(call: CallbackQuery, state: FSMContext):
    await state.update_data(category_id=int(call.data.split(":")[2]))
    await state.set_state(AdminState.edit_warning)
    await call.message.edit_text("Envoie le texte du warning :", reply_markup=cancel_kb())
    await call.answer()

@router.message(AdminState.edit_warning)
async def warning_save(message: Message, state: FSMContext):
    data = await state.get_data()
    async with SessionLocal() as session:
        cat = await session.get(Category, data["category_id"])
        cat.warning_text = message.text
        cat.warning_enabled = True
        await session.commit()
    await state.clear()
    await message.answer("Warning enregistré.", reply_markup=kb([[("Modération", "mod:menu")]]))
...
async def suggestions(call: CallbackQuery):
    ...
    for s in suggestions:
        text += f"#{s.id} — {s.name}\n"
        rows.append([(s.name, f"mod:sug:{s.id}")])
    rows.append([("Retour", "mod:menu")])
    await call.message.edit_text(text, reply_markup=kb(rows))
    await call.answer()

@router.callback_query(F.data.startswith("mod:sug:"))
async def suggestion_one(call: CallbackQuery):
    sid = int(call.data.split(":")[2])
    await call.message.edit_text("Que faire ?", reply_markup=kb([
        [("Valider", f"mod:sug_accept:{sid}")],
        [("Refuser avec motif", f"mod:sug_reject:{sid}")],
        [("Retour", "mod:suggestions")]
    ]))
    await call.answer()
...
async def sug_accept(call: CallbackQuery):
    ...
    await call.message.edit_text("Suggestion acceptée.", reply_markup=kb([[("Retour", "mod:suggestions")]]))
    await call.answer()
