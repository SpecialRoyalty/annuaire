...
async def vote(call:CallbackQuery):
    _,pid,value=call.data.split(":"); pid=int(pid); value=int(value)
    async with SessionLocal() as session:
        ...
        if exists: await call.answer("Tu as déjà voté.",show_alert=True); return
        p.rating_sum += value; p.rating_count += 1; session.add(DailyVote(project_id=pid,telegram_id=call.from_user.id)); await session.commit()
